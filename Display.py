...
def display(CLK,DIO,displayTerminate):
    """Confirm the display operation"""
    display = TM1637(CLK, DIO, brightness=1.0)
    display.Clear()
    display.ShowDoublepoint(True)
    digits = [1, 0, 0, 0]
    display.Show(digits)
    print("1000  - starting now")
    #2nd digit
    for minutes in range(9, -1, -1):
        display.Show1(0, 0)
        display.Show1(1, minutes)
        #3rd digit
        for seconds1 in range(5, -1, -1):
            display.Show1(2, seconds1)
            #4th digit
            for seconds2 in range(9, -1, -1):
                display.Show1(3, seconds2)
                if(displayTerminate.is_set()):
                    display.Clear()
                    display.cleanup()
                    return
                if(minutes==1 and seconds1 == 0 and seconds2 == 0):
                    sound.oneMin()
                elif(minutes==3 and seconds1 == 0 and seconds2 == 0):
                    sound.threeMin()
                elif(minutes==5 and seconds1 == 0 and seconds2 == 0):
                    sound.fiveMin()
                #1 seconds equals 0.05 seconds for testability
                sleep(0.05)
    print("0000  timer has ended")
    display.Clear()
    display.cleanup()